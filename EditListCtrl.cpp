...
BOOL CEditListCtrl::NonEditCell(int iRow, int iCol) {
    if (this->m_noEditRow.count(iRow) > 0) {
        return TRUE;
    }

    if (this->m_noEditCol.count(iCol) > 0) {
        return TRUE;
    }

    for (INT_SET_MAP_IT it = nonEdit.begin(); it != nonEdit.end(); it++) {
        NONEDIT_TYPE type = (*it).first;
        INT_SET & items = (*it).second;
        if (type == NONEDIT_BY_ROW) {
            if (items.find(iRow) != items.end())
                return TRUE;
        }
        else if (type == NONEDIT_BY_COL) {
            if (items.find(iCol) != items.end())
                return TRUE;
        }
    }
    return FALSE;
...
void CEditListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) {
    ...
    if (item != -1 && (lvhit.flags & LVHT_ONITEM) && (NonEditCell(lvhit.iItem, lvhit.iSubItem) == FALSE)) {
        m_iItem = lvhit.iItem;
...
void CEditListCtrl::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (item != -1 && (lvhit.flags & LVHT_ONITEM)) {
        ...
        if (m_bHighLight && m_iItem == lvhit.iItem && m_iSubItem == lvhit.iSubItem && (NonEditCell(lvhit.iItem, lvhit.iSubItem) == FALSE)) {
            //第二次单击
            EditLabel(m_iItem);
            return;
        }
...
void CEditListCtrl::SetNoEditCol(int iCol) {
    this->m_noEditCol[iCol] = false;
}

void CEditListCtrl::SetNoEditRow(int iRow) {
    this->m_noEditRow[iRow] = false;
}
